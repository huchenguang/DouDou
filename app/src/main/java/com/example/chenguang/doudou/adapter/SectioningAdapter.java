...
public class SectioningAdapter extends RecyclerView.Adapter<SectioningAdapter.ViewHolder> {
     ...
     * [0, 255] specifying a custom type for this footer.
    ...
    /**
     * For scenarios with multiple types of items, override this to return an integer in range [0,
     * 255] specifying a custom type for the item at this position
    ...
    /**
     * Called when a WrapperViewHolder is needed for a section item view
     *
    ...
    /**
     * Called when a WrapperViewHolder is needed for a section header view
     *
     * @param parent         The ViewGroup into which the new View will be added after it is bound
     *                       to an adapter position.
    ...
    /**
     * Called when a WrapperViewHolder is needed for a section footer view
     *
     * @param parent         The ViewGroup into which the new View will be added after it is bound
     *                       to an adapter position.
    ...
    /**
     * Called when a WrapperViewHolder is needed for a section ghost header view
     *
    ...
    public int getSectionForAdapterPosition(int adapterPosition) {
        ...
        if (adapterPosition < 0 || adapterPosition >= getItemCount()) {
            throw new IndexOutOfBoundsException("adapterPosition " + adapterPosition + " is not " +
                    "in range of items represented by adapter");
        }
    ...
    /**
     * Given a sectionIndex and an adapter position getWillPlaying the local position of an item relative to
     * the sectionIndex,
    ...
    public int getPositionOfItemInSection(int sectionIndex, int adapterPosition) {
        ...
        if (sectionIndex >= sections.size()) {
            throw new IndexOutOfBoundsException("sectionIndex " + sectionIndex + " >= sections" +
                    ".size (" + sections.size() + ")");
        }
        ...
        if (localPosition > section.length) {
            throw new IndexOutOfBoundsException("adapterPosition: " + adapterPosition + " is " +
                    "beyond sectionIndex: " + sectionIndex + " length: " + section.length);
        }
    ...
    public void toggleSectionItemSelected(int sectionIndex, int itemIndex) {
        setSectionItemSelected(sectionIndex, itemIndex, !isSectionItemSelected(sectionIndex,
                itemIndex));
    }
    ...
    private void notifySectionItemRangeRemoved(int sectionIndex, int fromPosition, int number,
        ...
        if (sections == null) {
            ...
            if (fromPosition + number > section.numberOfItems) {
                throw new IndexOutOfBoundsException("itemIndex adapterPosition: " + fromPosition
                        + number + " exceeds sectionIndex numberOfItems: " + section
                        .numberOfItems);
            }
     ...
     * @param itemIndex    the index of the item relative to the section (where 0 is the first
     *                     item in the section)
     */
    ...
    public void notifySectionFooterInserted(int sectionIndex) {
        if (sections == null) {
            ...
            if (!section.hasFooter) {
                throw new IllegalArgumentException("notifySectionFooterInserted: adapter " +
                        "implementation reports that section " + sectionIndex + " does not have a" +
                        " " +
                        "footer");
            }
    ...
    public void notifySectionFooterRemoved(int sectionIndex) {
        if (sections == null) {
            ...
            Section section = this.sections.get(sectionIndex);
            if (section.hasFooter) {
                throw new IllegalArgumentException("notifySectionFooterRemoved: adapter " +
                        "implementation reports that section " + sectionIndex + " has a footer");
            }
    ...
    public void notifySectionFooterChanged(int sectionIndex) {
        if (sections == null) {
            ...
            Section section = this.sections.get(sectionIndex);
            if (!section.hasFooter) {
                throw new IllegalArgumentException("notifySectionFooterChanged: adapter " +
                        "implementation reports that section " + sectionIndex + " does not have a" +
                        " " +
                        "footer");
            }
    ...
    private void updateSectionItemRangeSelectionState(int sectionIndex, int fromPosition, int
            delta) {
    ...
    public int getItemViewType(int adapterPosition) {
        ...
        if (adapterPosition < 0) {
            throw new IndexOutOfBoundsException("adapterPosition (" + adapterPosition + ") cannot" +
                    " be < 0");
        } else if (adapterPosition >= getItemCount()) {
            throw new IndexOutOfBoundsException("adapterPosition (" + adapterPosition + ")  " +
                    "cannot be > getItemCount() (" + getItemCount() + ")");
        }
        ...
        switch (baseType) {
            case TYPE_HEADER:
                ...
                if (userType < 0 || userType > 0xFF) {
                    throw new IllegalArgumentException("Custom header view type (" + userType +
            ...
            case TYPE_ITEM:
                ...
                if (userType < 0 || userType > 0xFF) {
                    throw new IllegalArgumentException("Custom item view type (" + userType + ") " +
                            "must be in range [0,255]");
                }
                break;
            case TYPE_FOOTER:
                ...
                if (userType < 0 || userType > 0xFF) {
                    throw new IllegalArgumentException("Custom footer view type (" + userType +
                            ") must be in range [0,255]");
                }
                break;
        }
     ...
     * @return the base type (TYPE_HEADER, TYPE_GHOST_HEADER, TYPE_ITEM, TYPE_FOOTER) of the item
     * at a given adapter position
     */
    ...
    public static int unmaskBaseViewType(int itemViewTypeMask) {
        return itemViewTypeMask & 0xFF; // base view type (HEADER/ITEM/FOOTER/GHOST_HEADER) is
        // lower 8 bits
    }
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        throw new IndexOutOfBoundsException("unrecognized viewType: " + viewType + " does not " +
                "correspond to TYPE_ITEM, TYPE_HEADER or TYPE_FOOTER");
    }
    ...
    public void onBindViewHolder(ViewHolder holder, int adapterPosition) {
        ...
        switch (baseType) {
            ...
            default:
                throw new IllegalArgumentException("unrecognized viewType: " + baseType + " does " +
                        "not correspond to TYPE_ITEM, TYPE_HEADER, TYPE_GHOST_HEADER or " +
                        "TYPE_FOOTER");
        }
    }

    /**
     * Tag the itemView of the view holder with information needed for the layout to do its
     * sticky positioning.
     * Specifically, it tags R.id.sectioning_adapter_tag_key_view_type to the item type, R.id
     * .sectioning_adapter_tag_key_view_section
     * to the item's section, and R.id.sectioning_adapter_tag_key_view_adapter_position which is
     * the adapter position of the view
     *
