...
import com.example.chenguang.doudou.adapter.SectioningAdapter;
...
public class StickyHeaderLayoutManager extends RecyclerView.LayoutManager {
     ...
     * This can be useful if client code wants to change appearance for headers in HeaderPosition
     * .STICKY vs normal positioning.
     *
     * @see HeaderPosition
     */
    public interface HeaderPositionChangedCallback {
         ...
         * HeaderPosition.NONE, HeaderPosition.NATURAL, HeaderPosition.STICKY or HeaderPosition
         * .TRAILING
         *
    ...
    public void onAdapterChanged(RecyclerView.Adapter oldAdapter, RecyclerView.Adapter newAdapter) {
        ...
        try {
            ...
            throw new ClassCastException("StickyHeaderLayoutManager must be used with a " +
                    "RecyclerView where the adapter is a kind of SectioningAdapter");
        }
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (state instanceof SavedState) {
            ...
            Log.e(TAG, "onRestoreInstanceState: invalid saved state class, expected: " +
                    SavedState.class.getCanonicalName() + " got: " + state.getClass()
                    .getCanonicalName());
        }
    }
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int adapterPosition = firstViewAdapterPosition; adapterPosition < state.getItemCount
            ...
            if (itemViewType == SectioningAdapter.TYPE_HEADER) {
                ...
                // we need to back up and getWillPlaying the header for this ghostHeader
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State
        ...
        if (dy < 0) {
            ...
            while (scrolled > dy) {
                ...
                // getWillPlaying the topmost view
                int hangingTop = Math.max(-getDecoratedTop(topView), 0);
                int scrollBy = Math.min(scrolled - dy, hangingTop); // scrollBy is positive,
                // causing content to move downwards
                ...
                if (firstViewAdapterPosition > 0 && scrolled > dy) {
                    ...
                    if (isGhostHeader) {
                        View header = createSectionHeaderIfNeeded(recycler, adapter
                                .getSectionForAdapterPosition(firstViewAdapterPosition));
                        top = bottom - getDecoratedMeasuredHeight(header); // header is already
                        // measured
                    } else {
                        measureChildWithMargins(v, 0, 0);
            ...
            while (scrolled < dy) {
                ...
                if (scrolled < dy && nextAdapterPosition < state.getItemCount()) {
                    ...
                    if (itemViewType == SectioningAdapter.TYPE_HEADER) {
                        ...
                        // getWillPlaying the header and measure it so we can followup immediately by
                        // vending the ghost header
                        View headerView = createSectionHeaderIfNeeded(recycler, adapter
                                .getSectionForAdapterPosition(nextAdapterPosition));
    ...
    /**
     * @param fullyVisibleOnly if true, the search will be limited to the first item not hanging
    ...
    @Nullable
    private SectioningAdapter.ViewHolder getFirstVisibleViewHolderOfType(int baseType, boolean
            fullyVisibleOnly) {
        if (getChildCount() == 0) {
            return null;
        }
    ...
    private void recycleViewsOutOfBounds(RecyclerView.Recycler recycler) {
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            if (getViewBaseType(view) == SectioningAdapter.TYPE_HEADER && !remainingSections
    ...
    private void recordHeaderPositionAndNotify(int sectionIndex, View headerView, HeaderPosition
            newHeaderPosition) {
            ...
            if (currentHeaderPosition != newHeaderPosition) {
                ...
                if (headerPositionChangedCallback != null) {
                    headerPositionChangedCallback.onHeaderPositionChanged(sectionIndex,
                            headerView, currentHeaderPosition, newHeaderPosition);
                }
    ...
    private SectioningAdapter.ViewHolder getViewViewHolder(View view) {
        return (SectioningAdapter.ViewHolder) view.getTag(R.id
                .sectioning_adapter_tag_key_view_view_holder);
    }
    ...
    private class SmoothScroller extends LinearSmoothScroller {
        ...
        public PointF computeScrollVectorForPosition(int targetPosition) {
            return new PointF(0, StickyHeaderLayoutManager.this.computeScrollVectorForPosition
                    (targetPosition));
        }
